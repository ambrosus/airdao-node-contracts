...
describe("Finance", function () {
  ...
  describe("withdraw", function () {
    ...
    it("masterfinance withdraw() not owner ", async function () {
      ...
      await expect(masterFinance.connect(notOwner).withdraw(owner.address, 420)).to.be.revertedWith(
    ...
    it("finance withdraw() not owner ", async function () {
      const [_, notOwner] = await ethers.getSigners();
      await expect(finance.connect(notOwner).withdraw(owner.address, 420)).to.be.revertedWith(
        "Ownable: caller is not the owner"
      );
    });
