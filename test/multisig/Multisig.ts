...
import { MasterMultisig__factory, Multisig, Multisig__factory, OnDemandRevert } from "../../typechain-types";
...
describe("Multisig", function () {
  ...
  describe("deploy", function () {
    it("must be at least 1 initiator", async function () {
      await expect(MultisigMasterFactory.deploy([addresses[0], addresses[1]], [false, false], 100)).to.be.revertedWith(
        "must be at least 1 initiator"
      );
    });
    it("required signers must be > 0", async function () {
      await expect(MultisigMasterFactory.deploy([], [], 100)).to.be.revertedWith("required signers must be > 0");
    });
  });

  describe("changeSigners", function () {
    it("not owner call should fail", async function () {
      await expect(multisig.connect(signers[1]).changeSigners([addresses[0]], [], [])).to.be.revertedWith(
    ...
    it("remove not existing signer should fail", async function () {
      await expect(multisig.changeSigners([addresses[1]], [], [])).to.be.revertedWith("Not a signer");
    });

    it("remove single signer should fail", async function () {
      await expect(multisig.changeSigners([addresses[0]], [], [])).to.be.revertedWith("required signers must be > 0");
    });

    it("remove all initiators should fail", async function () {
      await multisig.changeSigners([], [addresses[1]], [false]); // firstly add signer
      await expect(multisig.changeSigners([addresses[0]], [], [])).to.be.revertedWith("must be at least 1 initiator");
    });

    it("change isInitiator = false from single signer should fail", async function () {
      await expect(multisig.changeSigners([], [addresses[0]], [false])).to.be.revertedWith(
        "must be at least 1 initiator"
      );
    });

    it("add same signer with same isInitiator should fail", async function () {
      await expect(multisig.changeSigners([], [addresses[0]], [true])).to.be.revertedWith("Already signer");
    });
    ...
    it("different signersToAdd and isInitiatorFlags arrays length should fail", async function () {
      await expect(multisig.changeSigners([], [addresses[0]], [])).to.be.revertedWith(
        "signersToAdd.length != isInitiatorFlag.length"
      );
    });
  });
  ...
  describe("changeThreshold", function () {
    it("not owner call should fail", async function () {
      await expect(multisig.connect(signers[1]).changeThreshold(100)).to.be.revertedWith(
        "Ownable: caller is not the owner"
      );
    });

    it("value > 100 should fail", async function () {
      await expect(multisig.changeThreshold(101)).to.be.revertedWith("threshold must be <= 100");
    });

    it("all values should work; requiredSigners < 1 should fail", async function () {
      const signersToAdd = addresses.slice(1, 32);
      await multisig.changeSigners(
        [],
        signersToAdd,
        signersToAdd.map(() => false)
      );
  ...
  describe("submitTransaction", function () {
    ...
    it("not initiator submitTransaction call should fail", async function () {
      await expect(
        multisig.connect(signers[1]).submitTransaction(onDemandRevert.address, 0, calldata)
      ).to.be.revertedWith("Not a initiator");
    });

    it("0x0 destination should fail", async function () {
      await expect(multisig.submitTransaction(ethers.constants.AddressZero, 0, calldata)).to.be.revertedWith(
        "Destination can't be 0x0"
      );
    });

    it("msg.value != value should fail", async function () {
      await expect(multisig.submitTransaction(onDemandRevert.address, 42, calldata, { value: 32 })).to.be.revertedWith(
        "msg.value != value"
      );
    });
  ...
  describe("others", function () {
    it("withdraw", async function () {
      ...
      await expect(multisig.connect(signers[1]).withdraw(addresses[1], 42)).to.be.revertedWith(
        "Must be called from owner or from multisig"
      );
    ...
    it("checkBeforeSubmitTransaction", async function () {
      ...
      await expect(
        multisig.callStatic.checkBeforeSubmitTransaction(onDemandRevert.address, 0, calldataSuccess)
      ).to.be.revertedWith("OK");
      await expect(
        multisig.callStatic.checkBeforeSubmitTransaction(onDemandRevert.address, 0, calldataFail)
